...
function DriverProfile() {
  ...
  const [DriverProfile, setProfile] = useState(null); //profile is initially null, set after fetching from server
  ...
  useEffect(() => {
    ...
    const fetchFromServer = async () => {
      const result = await axios.get("http://localhost:3000/driver/profile", {
  ...
  const name = DriverProfile.name;
  const username = DriverProfile.username;
  const emirate = DriverProfile.emirate;
  const pno = DriverProfile.number;
  const plate = DriverProfile.number_plate;
      ...
      <div className="card">
        <div className="card-body">
          ...
          <p className="card-text">
            Driver ID: {username} <br />
            Emirate: {emirate} <br />
            Plate Number: {plate} <br />
            TEL: +971{pno}{" "}
          </p>
        </div>
      </div>
