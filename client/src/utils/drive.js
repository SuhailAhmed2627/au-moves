...
const getDirectionsForSlot = async (rides, slot, directionsService) => {
  ...
  return {
    ...
    route: directionsResponse1.routes[0],
    result: directionsResponse1,
  };
};

const createMapLink = (rides, result) => {
  const orderedRides = result.routes[0].waypoint_order.map(
    (index) => rides[index],
  );
  ...
  url.searchParams.append(
    "waypoints",
    orderedRides
      .map((ride) => `${ride.bookedBy.latitude},${ride.bookedBy.longitude}`)
      .join("|"),
  );
...
const getPickUpTimeForDriver = async (
  driverId,
  rides,
  slot,
  directionsService,
) => {
  const directionsData = await getDirectionsForSlot(
    rides,
    slot,
    directionsService,
  );
...
const getPickUpTimeForSlot = async (
  driverIdToRidesObject,
  directionsService,
  slot,
) => {
  const temp = {};
  for (const key in driverIdToRidesObject) {
    ...
    const data = await getPickUpTimeForDriver(
      driverId,
      rides,
      slot,
      directionsService,
